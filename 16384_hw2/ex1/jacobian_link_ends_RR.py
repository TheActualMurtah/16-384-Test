...
def jacobian_link_ends_RR(theta):
    ...
    c12, s12 = np.cos(theta_1 + theta_2), np.sin(theta_1 + theta_2)
    #pos_1 = np.array([l1 * np.cos(theta_1), l1 * np.sin(theta_1), theta_1])
    #pos_2 = np.array([l1 * c1 + l2 * c12, l1 * s1 + l2 * s12, theta1 + theta2])
    J_END_1 = np.array([[-l1 * s1, 0],[l1*c1, 0]])
    J_END_2 = np.array([[-l1 * s1 -l2 * s12, -l2*s12],[l1*c1 + l2*c12, l2*c12]])
    # --------------- END STUDENT SECTION ------------------------------------
