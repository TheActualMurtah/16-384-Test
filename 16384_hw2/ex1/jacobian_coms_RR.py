...
def jacobian_coms_RR(theta):
    ...
    c1,c12,s1,s12 = np.cos(theta1), np.cos(theta1 + theta2), np.sin(theta1), np.sin(theta1 + theta2)
    #pos 1 = [(l1/2) * c1, (l1/2) * s1] 
    #pos 2 = [l1*c1 + (l2/2)*c12, l1*s1 + (l2/2)*s12]
    J_COM_1 = np.array([[-(l1/2) * s1, 0],[(l1/2) * c1, 0]]) 
    J_COM_2 = np.array([[-l1*s1 - (l2/2)*s12, -(l2/2)*s12],[l1*c1 + (l2/2) * c12, (l2/2) * c12]])
    # --------------- END STUDENT SECTION ------------------------------------
