...
def main():
    ...
    direction = sp.Matrix([1,1])  # Define the direction vector
    grad_f = sp.Matrix([f_x, f_y]).subs([(x, 1), (y,2)])  # Define the gradient and substitute specific points
    dir_derivative = grad_f.dot(direction / direction.norm())  # Compute the directional derivative
